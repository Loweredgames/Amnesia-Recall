...
execute if entity @a[nbt={SelectedItem:{id:"minecraft:lantern",count:1,tag:{display:{Name:'{"text":"Lantern","color":"gold","bold":true}'},HideFlags:39}}}] run scoreboard players add @a lantern 1
execute at @a[scores={drop_lantern=1}] run item replace entity @a hotbar.0 with lantern[custom_name='{"bold":true,"color":"gold","text":"Lantern"}',hide_additional_tooltip={}] 1
execute at @a[scores={drop_lantern=1..}] run item replace entity @a hotbar.0 with quartz[custom_name='{"bold":true,"color":"dark_gray","italic":true,"strikethrough":true,"text":"Lantern (OFF)"}'] 1
execute at @a[scores={drop_lantern=1..}] run scoreboard players set @a drop_lantern 0
execute at @a[scores={drop_quartz=1..}] run item replace entity @a hotbar.0 with lantern[custom_name='{"bold":true,"color":"gold","text":"Lantern"}',hide_additional_tooltip={}] 1
execute at @a[scores={drop_quartz=1..}] run scoreboard players set @a drop_quartz 0
execute at @a[scores={drop_lantern=0..}] run kill @e[type=item,nbt={Item:{id:"minecraft:lantern",count:1}}]
execute at @a[scores={drop_quartz=0..}] run kill @e[type=item,nbt={Item:{id:"minecraft:quartz",count:1}}]
...
execute at @a[scores={oil=0}] run item replace entity @a hotbar.0 with quartz[custom_name='{"bold":true,"color":"dark_gray","italic":true,"strikethrough":true,"text":"Lantern (OFF)"}'] 1
